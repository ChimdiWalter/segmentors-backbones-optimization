...
def parse_args():
    ...
    p.add_argument(
        "--root",
        default="experiments/exp_v1/outputs/objective_surface_traces_representative_fullbudget",
        help="Output root for the representative trace study (original settings)",
    )
    return p.parse_args()
...
def main():
    ...
    # Runtime from rerun opt summary
    rerun_runtimes = {}
    if "elapsed_seconds" in rerun_df.columns:
        for _, rr in rerun_df.iterrows():
            rerun_runtimes[str(rr["filename"]).strip()] = float(rr["elapsed_seconds"])
    total_runtime = sum(rerun_runtimes.values())

    lines = []
    lines.append("True Trace-Based Objective-Surface Study Report")
    lines.append("====================================================")
    lines.append("")
    lines.append("IMPORTANT: This is a representative rerun using the ORIGINAL optimizer settings.")
    lines.append("Only the number of leaves was reduced (from 174 to 6).")
    lines.append("All search budget parameters are identical to the production run.")
    lines.append("")
    lines.append("Original optimizer settings (from navier_optimize_hellbender_updatecsv.py):")
    lines.append("  budget-random       = 64")
    lines.append("  topk-refine         = 4")
    lines.append("  refine-steps        = 12")
    lines.append("  iters-coarse        = 16")
    lines.append("  snake-iters-coarse  = 60")
    lines.append("  iters-final         = 30")
    lines.append("  snake-iters-final   = 100")
    lines.append("  downscale           = 0.75")
    lines.append("  mode                = fused")
    ...
    for _, r in selected.iterrows():
        rt = rerun_runtimes.get(str(r["filename"]).strip(), float("nan"))
        lines.append(f"- {r['group']}: {r['filename']} (original score={float(r['score']):.6f}, runtime={rt:.1f}s)")
    lines.append("")
    lines.append(f"Total runtime: {total_runtime:.1f}s ({total_runtime/60:.1f} min)")
    if len(rerun_runtimes):
        lines.append(f"Mean runtime per leaf: {total_runtime/len(rerun_runtimes):.1f}s")
    lines.append("")
    ...
    lines.append("")
    lines.append("Recommendation: expand from 6 to 9 leaves?")
    lines.append("-------------------------------------------")
    if len(valid):
        n_sharp = int((valid["sharpness"] == "sharp").sum())
        n_basin = int(valid["multi_basin_hint"].sum())
        if n_sharp >= 4 and n_basin >= 2:
            lines.append("- The 6-leaf sample already shows diverse landscape geometry (sharp surfaces, multiple basins).")
            lines.append("- Expanding to 9 leaves would add statistical confidence but may not qualitatively change conclusions.")
            lines.append("- RECOMMENDATION: 6 leaves likely sufficient; expand to 9 only if quantitative error bars are needed.")
        else:
            lines.append("- Some landscape features are underrepresented in the 6-leaf sample.")
            lines.append("- RECOMMENDATION: Expand to 9 leaves (add 1 more from each tier) for better coverage.")
    lines.append("")
