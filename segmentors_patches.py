...
- Backbones: color, resnet, deeplab, dinov2(518-preprocess), swav, moco(optional), mae(224-preprocess), sam.
...
- DINOv2: force 518 resize+centercrop (works on any tile).
...
def cluster_to_lesions(
    ...
    if k_open.size>0:  m = cv.morphologyEx(m, cv.MORPH_OPEN,  k_open,  iterations=1)
    if k_close.size>0: m = cv.morphologyEx(m, cv.MORPH_CLOSE, k_close, iterations=1)
...
def main():
    ...
    built = build_backbones(requested, args.sam_checkpointh if False else args.sam_checkpoint, moco_local)  # keep arg name
