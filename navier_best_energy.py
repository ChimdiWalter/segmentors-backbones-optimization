...
OUTPUT_DIR = '/deltos/e/lesion_phes/code/python/pipeline/segmentors_backbones/navier_output_best'
...
# ---------------------------------------
# Helpers for dtype-safe I/O and scaling
# ---------------------------------------
...
def auto_eth_dual(grad_mag_u8, leaf_mask_u8):
    ...
    if rng < 20 or Eth_seed <= Eth_q + 2:
        # fallback to Otsu when contrast is poor
...
def snake_seg(image_u8, energy_u8, its, alpha, beta, gamma, l_size, u_size, energy_threshold):
    ...
    for rgn in props:
        ...
        # 200-point circular init (faster than 400)
...
# ---------------------------------------
# Best-channel selector (+ CLAHE later)
# ---------------------------------------
...
def process_single_image(filename):
    ...
    try:
        ...
        # Best channel + CLAHE on leaf only
        best_ch = best_single_channel_u8(bgr8, binary_mask)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        best_enh = clahe.apply(best_ch)
        masked_best = cv2.bitwise_and(best_enh, best_enh, mask=binary_mask)

        # Auto thresholds per image
        grad_mag_u8, gx, gy = gradient_magnitude(masked_best)
        Eth_q, Eth_seed = auto_eth_dual(grad_mag_u8, binary_mask)
        print(f"[AUTO] {filename}: Eth_q={Eth_q}, Eth_seed={Eth_seed}", flush=True)
        ...
        for (mu, lam, dr, alpha, beta, gamma) in COMBOS:
            ...
            try:
                ...
                # snakes (50 iters, 200-pt init, skip if 0 regions inside snake_seg)
                seg_mask_bool = snake_seg(
                    masked_best, diff_map, its=50,
                    alpha=alpha, beta=beta, gamma=gamma,
                    l_size=25, u_size=50000,  # filter tiny noise
...
def process_images_with_grid_search(input_dir, output_dir, log_csv="results_log.csv"):
    ...
    print("========== navier_bestch AUTO PARALLEL DEBUG ==========")
