#!/usr/bin/env python3
# nv_optimize_robust_v3_gpu_fast.py
"""
Faster robust_v3 without changing the objective:
- Batched GPU energy per candidate batch
- Cheap pre-sieve (no snakes) -> snake only top-K per batch
- Final full-res snakes unchanged
"""
...
# Torch is required for GPU path; CPU fallback supported
import torch
...
from scipy.ndimage import laplace as _laplace_cpu
...
BOUNDS = {
    ...
    "beta":             (0.05, 1.20),
    "gamma":            (0.01, 0.60),
    "energy_threshold": (15, 140),
}

# Robust objective (unchanged)
...
def _make_conv(weight_2d: torch.Tensor, device):
    k = weight_2d.to(device=device, dtype=torch.float32).view(1,1,weight_2d.shape[0], weight_2d.shape[1])
    return k
...
def _laplace_kernel(device):
    return _make_conv(torch.tensor([[0,1,0],[1,-4,1],[0,1,0]], dtype=torch.float32), device)

@torch.no_grad()
def gpu_gradients(img_u8: np.ndarray, device: str, amp: bool=False):
    dev = torch.device(device)
    x = torch.from_numpy(img_u8.astype(np.float32)/255.0).to(dev)[None,None,...]  # [1,1,H,W]
    kx, ky = _sobel_kernels(dev)
    ctx = torch.autocast(device_type="cuda", dtype=torch.float16) if (amp and device=="cuda") else nullcontext()
    with ctx:
        gx = F.conv2d(x, kx, padding=1)
        gy = F.conv2d(x, ky, padding=1)
        mag = torch.sqrt(gx*gx + gy*gy).clamp_min(1e-8)
    gx_np = gx.squeeze().float().cpu().numpy()
    gy_np = gy.squeeze().float().cpu().numpy()
    mag_u8 = (mag.squeeze().float().cpu().numpy() / float(mag.max().item()) * 255.0).astype(np.uint8)
...
def gpu_energy_maps_batched(
    ...
    mu_vec: np.ndarray,
    lam_vec: np.ndarray,
    edge_thresh_vec: np.ndarray,
    device: str,
    amp: bool=False
) -> np.ndarray:
    """
    Compute energy maps for B parameter sets in one pass.
    Returns uint8 array of shape (B, H, W).
    """
    dev = torch.device(device)
    H, W = image_u8.shape
    B = int(len(dr_vec))

    img = torch.from_numpy(image_u8.astype(np.float32)/255.0).to(dev)[None,None,...]   # 1x1xHxW
    s_gx = torch.from_numpy(seed_gx.astype(np.float32)).to(dev)[None,None,...]         # 1x1xHxW
    s_gy = torch.from_numpy(seed_gy.astype(np.float32)).to(dev)[None,None,...]         # 1x1xHxW

    # Expand to B
    imgB = img.expand(B, -1, -1, -1)             # Bx1xHxW
    sgxB = s_gx.expand(B, -1, -1, -1)            # Bx1xHxW
    sgyB = s_gy.expand(B, -1, -1, -1)

    kx, ky = _sobel_kernels(dev)
    lap_k = _laplace_kernel(dev)

    # Scalars as (B,1,1,1)
    dr  = torch.tensor(dr_vec,  device=dev, dtype=torch.float32).view(B,1,1,1)
    mu  = torch.tensor(mu_vec,  device=dev, dtype=torch.float32).view(B,1,1,1)
    lam = torch.tensor(lam_vec, device=dev, dtype=torch.float32).view(B,1,1,1)
    et  = torch.tensor(edge_thresh_vec, device=dev, dtype=torch.float32).view(B,1,1,1)/255.0

    # compute shared grads
    ctx = torch.autocast(device_type="cuda", dtype=torch.float16) if (amp and device=="cuda") else nullcontext()
    with ctx:
        im_gx = F.conv2d(imgB, kx, padding=1)
        im_gy = F.conv2d(imgB, ky, padding=1)
        im_mag = torch.sqrt(im_gx*im_gx + im_gy*im_gy)
        q = (im_mag > et).float()
        diffused = torch.sqrt(sgxB*sgxB + sgyB*sgyB).clone()

        for _ in range(int(iterations)):
            lap   = F.conv2d(diffused, lap_k, padding=1)
            # simple divergence proxy via ky (same as original robust_v3)
            div_v = F.conv2d(diffused, ky,     padding=1)
            ftx, fty = im_gx - sgxB, im_gy - sgyB
            ftm = torch.sqrt(ftx*ftx + fty*fty)
            diffused = diffused + dr * (mu * lap + (lam + mu) * div_v + q * (ftm - diffused))

        mx = diffused.flatten(1).max(dim=1).values.clamp_min(1e-8)  # B
        out = (diffused / mx.view(B,1,1,1) * 255.0).squeeze(1).float().cpu().numpy().astype(np.uint8)  # (B,H,W)
    return out
...
def _cheap_proxy_score(mask_u8: np.ndarray, target_area_frac=TARGET_AREA_FRAC) -> Tuple[float, int, int, float, int]:
    """
    Fast CC stats (no snakes) to rank candidates inside a batch.
    Uses the *same formula* as robust objective, only difference is region
    shapes are not refined by snakes in this step.
    """
    h, w = mask_u8.shape
    total = float(h*w + 1e-8)

    # connected components via OpenCV (fast)
    ...
    if area_frac > MAX_AREA_FRAC_HARD or area_frac < MIN_AREA_FRAC_HARD:
        return -1e9, 0, area_px, area_frac, 0

    n_contours = int((areas > 0).sum())
    small_count = int((areas < SMALL_MIN_AREA).sum())
    score = (n_contours
             - W_SMALL * small_count
             - W_AREA * abs(area_frac - target_area_frac))
    return float(score), n_contours, area_px, float(area_frac), small_count
...
def evaluate_params_cpu_seg(image_u8: np.ndarray,
    ...
    lab_ = label(out_mask)
    props_ = regionprops(lab_)
    small_count = sum(1 for p in props_ if p.area < SMALL_MIN_AREA)
...
def process_one_image(filename: str,
    ...
    work = cv2.resize(work_full, None, fx=downscale, fy=downscale, interpolation=cv2.INTER_AREA) \
        if 0.0 < downscale < 1.0 else work_full

    # base grads
    if device == "cuda":
        _, gx, gy = gpu_gradients(work, device=device, amp=amp)
    else:
        _, gx, gy = _cpu_gradients(work)
    ...
    def time_up():
        return per_image_seconds and (time.time() - t0 > per_image_seconds)

    # -------------- Random exploration with batched energy + pre-sieve --------
    while tried < budget_random:
        B = min(max(1, gpu_batch), budget_random - tried)
        cand_list = [clip_params({**sample_candidate(rng), "iters": iters_coarse}) for _ in range(B)]

        if device == "cuda":
            dr  = np.array([c["diffusion_rate"] for c in cand_list], np.float32)
            mu  = np.array([c["mu"] for c in cand_list], np.float32)
            lam = np.array([c["lambda"] for c in cand_list], np.float32)
            et  = np.array([int(c["energy_threshold"]) for c in cand_list], np.float32)
            energies = gpu_energy_maps_batched(work, gx, gy, iters_coarse, dr, mu, lam, et, device, amp=amp)
        else:
            energies = []
            for c in cand_list:
                energies.append(_cpu_energy(work, gx, gy, c))
            energies = np.stack(energies, axis=0)

        # pre-sieve: quick score (no snakes), then snake only top-K
        prelim = []
        for b in range(B):
            thr = int(cand_list[b]["energy_threshold"])
            mask = (energies[b] > thr).astype(np.uint8) * 255

            # quick reject huge masks (accelerator)
            num_labels, _, stats, _ = cv2.connectedComponentsWithStats((mask>0).astype(np.uint8), connectivity=8)
            if num_labels-1 > coarse_max_comps:
                prelim.append((-1e9, -1, -1, 0.0, 0))
                continue
            prelim.append(_cheap_proxy_score(mask))

        order = np.argsort([-p[0] for p in prelim])  # descending by proxy score
        snake_idxs = order[:min(sieve_topk, B)]

        # Coarse snakes for top-K only (true objective)
        for idx in snake_idxs:
            c = cand_list[idx]
            thr = int(c["energy_threshold"])
            res = evaluate_params_cpu_seg(work, energies[idx],
                                          its_snake=snake_iters_coarse,
                                          alpha=c["alpha"], beta=c["beta"], gamma=c["gamma"],
                                          l_size=5, u_size=u_size, energy_threshold=thr)
            tried += 1
            if (best is None) or (res.score > best[0].score):
                best = (res, dict(c))
            if time_up():
                return _finalize(filename, raw, work_full, best, mode, output_dir,
                                 iters_final, snake_iters_final, device, amp, timeout=True)

        # Count the rest as "tried" for budget purposes
        tried = max(tried, tried + (B - len(snake_idxs)))
        if time_up():
            return _finalize(filename, raw, work_full, best, mode, output_dir,
                             iters_final, snake_iters_final, device, amp, timeout=True)

    # -------------- Local refinement (same sieve) ------------------------------
    seeds = [best[1]]
    ...
    for seed_p in seeds:
        ...
        for step in range(refine_steps):
            ...
            # make a small batch around current
            pb = [clip_params(perturb(rng, current[1], scale)) for _ in range(min(gpu_batch, 16))]

            if device == "cuda":
                dr  = np.array([c["diffusion_rate"] for c in pb], np.float32)
                mu  = np.array([c["mu"] for c in pb], np.float32)
                lam = np.array([c["lambda"] for c in pb], np.float32)
                et  = np.array([int(c["energy_threshold"]) for c in pb], np.float32)
                energies = gpu_energy_maps_batched(work, gx, gy, iters_coarse, dr, mu, lam, et, device, amp=amp)
            else:
                energies = []
                for c in pb:
                    energies.append(_cpu_energy(work, gx, gy, {**c, "iters": iters_coarse}))
                energies = np.stack(energies, axis=0)

            prelim = []
            for b in range(len(pb)):
                thr = int(pb[b]["energy_threshold"])
                mask = (energies[b] > thr).astype(np.uint8) * 255
                prelim.append(_cheap_proxy_score(mask))
            idxs = np.argsort([-p[0] for p in prelim])[:min(sieve_topk, len(pb))]

            improved = False
            for j in idxs:
                c = pb[j]
                thr = int(c["energy_threshold"])
                res = evaluate_params_cpu_seg(work, energies[j],
                                              its_snake=snake_iters_coarse,
                                              alpha=c["alpha"], beta=c["beta"], gamma=c["gamma"],
                                              l_size=5, u_size=u_size, energy_threshold=thr)
    ...
    # -------------- Finalize at full resolution (unchanged) -------------------
    return _finalize(filename, raw, work_full, best, mode, output_dir,
                     iters_final, snake_iters_final, device, amp, timeout=False)

# ----------------------------- CPU helpers -----------------------------------
def _cpu_gradients(u8: np.ndarray):
...
def _cpu_energy(img_u8, base_gx, base_gy, cdict):
    iters = int(cdict["iters"])
    mu = float(cdict["mu"]); lam = float(cdict["lambda"]); dr = float(cdict["diffusion_rate"])
    ethr = int(cdict["energy_threshold"])
    ...
    q = (im_mag > ethr).astype(np.float64)
    for _ in range(iters):
        lap = _laplace_cpu(diffused)
...
def _finalize(filename, raw, full_img_u8, best, mode, output_dir,
              iters_final, snake_iters_final, device, amp, timeout: bool):
    if best is None:
        return {"filename": filename, "status": "no_candidates"}

    # full-res energy
    if device == "cuda":
        _, gx_f, gy_f = gpu_gradients(full_img_u8, device=device, amp=amp)
        params = dict(best[1]); params["iters"] = iters_final
        energy = gpu_energy_maps_batched(
            full_img_u8, gx_f, gy_f, iterations=iters_final,
            dr_vec=np.array([params["diffusion_rate"]], np.float32),
            mu_vec=np.array([params["mu"]], np.float32),
            lam_vec=np.array([params["lambda"]], np.float32),
            edge_thresh_vec=np.array([int(params["energy_threshold"])], np.float32),
            device=device, amp=amp
        )[0]
    else:
        _, gx_f, gy_f = _cpu_gradients(full_img_u8)
        params = dict(best[1]); params["iters"] = iters_final
        energy = _cpu_energy(full_img_u8, gx_f, gy_f, params)
...
# ----------------------------- CLI / Driver ----------------------------------
def main():
    ap = argparse.ArgumentParser(description="FAST robust_v3: batched GPU energy + pre-sieve; same final objective.")
    ...
    ap.add_argument("--iters-coarse", type=int, default=16, help="Energy iters during search")
    ap.add_argument("--snake-iters-coarse", type=int, default=60, help="Snake iters during search")
    ap.add_argument("--iters-final", type=int, default=30, help="Energy iters at final full-res")
    ap.add_argument("--snake-iters-final", type=int, default=100, help="Snake iters at final full-res")

    ap.add_argument("--downscale", type=float, default=0.75, help="Search scale (0,1]; final is full-res")
    ap.add_argument("--limit", type=int, default=None, help="Max images")

    ap.add_argument("--workers", type=int, default=None, help="CPU workers (ignored on CUDA)")
    ap.add_argument("--per-image-seconds", type=int, default=None, help="Watchdog per image (s)")
    ...
    ap.add_argument("--gpu-batch", type=int, default=16, help="Candidates per GPU batch")
    ap.add_argument("--sieve-topk", type=int, default=4, help="Coarse: snake only top-K per batch")
    ap.add_argument("--coarse-max-comps", type=int, default=20000, help="Skip coarse masks with too many CCs")
    ap.add_argument("--amp", action="store_true", help="Mixed precision for GPU energy")

    args = ap.parse_args()

    # Guard threads in CPU parts
    ...
    if args.device == "cuda" and not torch.cuda.is_available():
        print("[warn] --device cuda requested but no CUDA; falling back to CPU.", flush=True)
        args.device = "cpu"
    ...
    max_workers = 1 if use_cuda else (args.workers if args.workers is not None else max(1, (os.cpu_count() or 2) - 1))

    print("========== nv_optimize_robust_v3_gpu_fast ==========")
    print(f"Input:    {input_dir}")
    print(f"Output:   {output_dir}")
    print(f"Mode:     {args.mode}")
    print(f"Images:   {len(files)} / {total} total")
    print(f"Workers:  {max_workers}{' (sequential on CUDA)' if use_cuda else ''}")
    print(f"Device:   {args.device}   (gpu-batch={args.gpu_batch}, amp={args.amp})")
    print(f"Search:   rand={args.budget_random}, topk={args.topk_refine}, steps={args.refine_steps}, downscale={args.downscale}")
    print(f"Coarse:   iters={args.iters_coarse}, snake={args.snake_iters_coarse}, sieve_topk={args.sieve_topk}")
        ...
        for i, fn in enumerate(files, 1):
            try:
                out = process_one_image(
                    ...
                    args.device, args.gpu_batch, args.sieve_topk,
                    args.coarse_max_comps, args.amp
                )
            except Exception as e:
                out = {"filename": fn, "status": f"worker_error:{e}"}
            rows.append(out)
            print(f"[{i}/{len(files)}] {fn}: {out.get('status','?')}  score={out.get('score','-')}", flush=True)
    else:
        # CPU path: optional spawned pool
