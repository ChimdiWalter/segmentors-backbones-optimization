...
TRACE_FIELDS = [
    "case_id", "source_image", "phase", "iteration", "candidate_index", "is_refine",
    "mu", "lambda", "diffusion_rate", "alpha", "beta", "gamma", "energy_threshold",
    "score", "best_score_so_far", "elapsed_seconds",
    "area_px", "n_contours", "area_frac", "status",
]
...
def _trace_status(score: float) -> str:
    return "ok" if score > -1e8 else "invalid_score"

def _write_trace_csv(path: str, rows: List[Dict]) -> None:
...
def process_one_image(filename, input_dir, output_dir, mode, budget_random, topk_refine, refine_steps, iters_coarse, snake_iters_coarse, iters_final, snake_iters_final, downscale, per_image_seconds, seed, trace_dir):
    rng = random.Random((hash(filename) ^ seed) & 0xFFFFFFFF)
    in_path = os.path.join(input_dir, filename)
    case_id = os.path.splitext(os.path.basename(filename))[0]
    trace_path = os.path.join(trace_dir, f"{case_id}.csv")
    ...
    best = None
    tried = 0
    refine_iter_global = 0

    def log_eval(phase, iteration, is_refine, cand, res):
        nonlocal candidate_index
        candidate_index += 1
        best_score = best[0].score if best is not None else float(res.score)
        trace_rows.append({
            "case_id": case_id,
            ...
            "is_refine": int(bool(is_refine)),
            "mu": float(cand["mu"]),
            "lambda": float(cand["lambda"]),
            "diffusion_rate": float(cand["diffusion_rate"]),
            "alpha": float(cand["alpha"]),
            "beta": float(cand["beta"]),
            "gamma": float(cand["gamma"]),
            "energy_threshold": int(cand["energy_threshold"]),
            "score": float(res.score),
            "best_score_so_far": float(best_score),
            "elapsed_seconds": float(time.time() - t0),
            "area_px": int(res.area_px),
            "n_contours": int(res.n_contours),
            "area_frac": float(res.area_frac),
            "status": _trace_status(float(res.score)),
        })

    def finalize_with_trace(timeout_flag):
        _write_trace_csv(trace_path, trace_rows)
        return _finalize(
            filename, raw, bgr8_full, work_full, best, mode, output_dir,
            iters_final, snake_iters_final, timeout_flag, time.time() - t0
        )
    ...
    while tried < budget_random:
        ...
        log_eval("random", tried, False, cand, res)
        if per_image_seconds and (time.time() - t0 > per_image_seconds):
            return finalize_with_trace(True)
    ...
    for s_idx, seed_p in enumerate(seeds):
        ...
        for step in range(refine_steps):
            ...
            res = evaluate_params_refined(work, img_mag, img_gx, img_gy, seed_mag, seed_gx, seed_gy, grad_mag_u8, lab_work, snake_iters_coarse, cand)
            tried += 1
            refine_iter_global += 1
            if res.score > current[0].score + 1e-6:
                current = (res, cand)
                patience = 0
                if best is None or res.score > best[0].score + 1e-6: best = (res, cand)
            else: patience += 1
            log_eval("refine", refine_iter_global, True, cand, res)
            if per_image_seconds and (time.time() - t0 > per_image_seconds):
                return finalize_with_trace(True)
            if patience >= 5: break

    return finalize_with_trace(False)
...
def main():
    ...
    ap.add_argument("--manifest", default=None, help="Optional text file listing image filenames to process")
    ap.add_argument("--workers", type=int, default=None)
    ap.add_argument("--per-image-seconds", type=int, default=None)
    ap.add_argument("--log", default="opt_summary.csv")
    ap.add_argument("--trace-dir", default=None, help="Directory for per-image candidate trace CSVs")
    ap.add_argument("--trace-merged", default=None, help="Merged candidate trace CSV path")
    ...
    if args.manifest:
        with open(args.manifest, "r") as f:
            wanted = [ln.strip() for ln in f if ln.strip() and not ln.strip().startswith("#")]
        wanted_set = set(wanted)
        wanted_stems = set(os.path.splitext(os.path.basename(x))[0] for x in wanted)
        files = [f for f in files if (f in wanted_set) or (os.path.splitext(f)[0] in wanted_stems)]
    if args.limit: files = files[:args.limit]
    max_workers = args.workers if args.workers else max(1, (os.cpu_count() or 2) - 1)
    trace_dir = args.trace_dir if args.trace_dir else os.path.join(args.output, "candidate_traces")
    ...
    if max_workers == 1:
        for i, fn in enumerate(files, 1):
            try:
                out = process_one_image(
                    fn, args.input, args.output, args.mode, args.budget_random, args.topk_refine,
                    args.refine_steps, args.iters_coarse, args.snake_iters_coarse, args.iters_final,
                    args.snake_iters_final, args.downscale, args.per_image_seconds, args.seed, trace_dir
                )
        ...
        try:
            with ProcessPoolExecutor(max_workers=max_workers) as ex:
                futs = [ex.submit(process_one_image, fn, args.input, args.output, args.mode, args.budget_random, args.topk_refine, args.refine_steps, args.iters_coarse, args.snake_iters_coarse, args.iters_final, args.snake_iters_final, args.downscale, args.per_image_seconds, args.seed, trace_dir) for fn in files]
            ...
            print("ProcessPool unavailable in this environment; falling back to serial execution.")
            for i, fn in enumerate(files, 1):
                try:
                    out = process_one_image(
                        fn, args.input, args.output, args.mode, args.budget_random, args.topk_refine,
    ...
    merged_path = args.trace_merged if args.trace_merged else os.path.join(os.path.dirname(trace_dir), "candidate_traces_merged.csv")
    trace_files = sorted([os.path.join(trace_dir, f) for f in os.listdir(trace_dir) if f.lower().endswith(".csv")])
    with open(merged_path, "w", newline="") as fout:
        writer = csv.DictWriter(fout, fieldnames=TRACE_FIELDS)
        writer.writeheader()
        for tf in trace_files:
            with open(tf, "r", newline="") as fin:
                reader = csv.DictReader(fin)
                for row in reader:
                    writer.writerow(row)
    print(f"Done. Merged trace CSV: {merged_path}")
