...
% --- FIX 1: EXPANDED CONTRIBUTIONS LIST ---
...
% --- FIX 2: REMOVED PLANNED-STUDY LANGUAGE ---
...
\begin{itemize}
    ...
    \item \textbf{Sparse GT (subset):} a stratified subset of images is used for Dice/IoU, Boundary-F1, and Hausdorff distance where teacher pseudo-labels serve as reference.
    \item \textbf{Cross-condition generalization:} images are stratified by date/camera tray/genotype to test robustness, not IID random splits.
\end{itemize}
...
% --- FIX 2 (BASELINES): REPLACED FORWARD-LOOKING WITH ACTIVE DESCRIPTION ---
...
% --- FIX 2 (METRICS): FORWARD-LOOKING LANGUAGE REMOVED ---
\subsection{Metrics}
...
\begin{itemize}
    ...
    \item Comparison against externally annotated lesion benchmarks is reserved as future work.
\end{itemize}
...
% --- FIX 3 APPENDIX: THRESHOLD SWEEP AND FAILURE-TAIL DETAILS ---
\section{Teacher--Student Threshold Sweep Details}
\label{sec:appendix_distillation_details}
...
\begin{itemize}
    \item $\text{Dice}_{\text{mean,all}}=0.604$, $\text{IoU}_{\text{mean,all}}=0.463$
    \item ROI-gated at threshold 0.30: mean Dice $0.599\pm0.225$ (all), $0.644\pm0.160$ (teacher-nonempty)
\end{itemize}
...
\subsection{Failure-Tail Analysis}
Failure-tail counts remain similar across the baseline and ROI-gated variants:
\begin{itemize}
    \item Baseline: 18 cases with Dice$<0.3$, including 12 teacher-empty/student-nonempty disagreements.
    \item ROI-gated: 19 cases with Dice$<0.3$, including 12 teacher-empty/student-nonempty disagreements.
\end{itemize}
