...
\medskip\noindent*Keywords:* Active Contours, Navier-Stokes Equations, Stochastic Optimization, Unsupervised Segmentation, Plant Phenotyping, Computer Vision in Agriculture.
...
In this paper, we bridge physics-based modeling and automation by proposing a /self-configuring/ framework that searches for per-image physics parameters and yields high-fidelity pseudo-labels for downstream learning.


*Contributions.* We provide:

  1. A *Navier-Stokes diffusion engine* that treats image gradients as a fluid, producing a smoothed energy landscape that supports weak boundaries and reduces initialization sensitivity.
  2. A *no-ground-truth composite objective* combining (i) gradient alignment, (ii) chromatic separability in Lab space, and (iii) connected-component/topology priors to discourage trivial solutions and encourage biologically plausible lesion multiplicity—-this objective is the primary mechanism that makes annotation-free optimization possible.
  3. A *gradient-free stochastic optimizer* (random-restart + hill climbing) that maximizes the objective per image over a bounded parameter space, with practical runtime controls (downscaling, timeouts, parallelism). Ablations confirm gradient alignment ($-34%$ without it) and topological reward ($-75%$ without it) are the most critical terms.
  4. A *trace-based objective-surface reconstruction and candidate-trace study*: by logging all candidate evaluations during optimization, we reconstruct projected pairwise surfaces $\mathcal{S}_{p,q}$ that characterize the non-convex objective landscape. Ablation of the surface sensitivity to energy threshold reveals that high-score leaves present exploitable curvature while low-score leaves yield flat surfaces, informing search-budget allocation strategies.
  5. A *parameter-cloud geometry analysis*: per-image optimal parameters $\theta^\star$ are analyzed via PCA, kNN classification, covariance ellipsoids, convex hulls, and solid-3D visualizations. Two PCs capture 46.5% of variance; kNN group agreement is 0.534 (vs.\ random baseline 0.33); centroid shift between global and local optimization clouds is 1.616 standardized units—-confirming systematic, non-random organization of the optimizer's solution space.
  6. A *teacher—student distillation pipeline* (secondary application): as a consequence of the structured parameter manifold, a patch-based UNet student can be trained on optimizer pseudo-labels without any manual annotation. Mean Dice $0.642\pm0.163$ on teacher-nonempty images. This result demonstrates practical amortization but is explicitly secondary to the optimizer and geometry contributions.
...
High-throughput phenotyping requires repeatable pipelines that produce interpretable traits, not just segmentations. Reviews highlight the role of image analysis in enabling scalable plant trait measurement [cite:gehan2017high,mahlein2016plant], while toolkits such as PlantCV standardize workflows and facilitate reproducibility [cite:plantcv2025]. Machine learning has been applied to high-throughput stress phenotyping with growing emphasis on automated feature extraction [cite:singh2016machine]. Our pipeline outputs both masks and rich per-image metadata (physics parameters, objective terms, runtime), aligning segmentation with downstream trait extraction (lesion count, area distributions, necrotic/chlorotic ratios).
...
We parameterize our physics engine by $\theta = (\mu,\lambda,\alpha,\beta,\gamma,T)$ where:
  ...
  - $(\mu,\lambda)$ govern diffusion/elastic coupling (Navier/elastic coefficients),
  - $(\alpha,\beta)$ are snake tension/rigidity,
  - $\gamma$ is an inflation/balloon force,
  - $T$ is an energy threshold controlling candidate region proposals.
...
where $S$ is a heuristic, no-ground-truth score (Section [[sec:objective]]) and $\Theta$ is a bounded search space.
...
We define the /candidate trace set/
...
Because $\theta\in\R^7$ and visualization in seven dimensions is impractical, we analyze /projected pairwise surfaces/: for any pair $(p,q)$ of parameters,
...
yielding a scatter-surface in $\R^3$ that reveals the objective landscape projected onto those two axes. This is distinct from geometric embeddings (e.g., PCA/UMAP) of parameter vectors, which capture positional spread but not score information. By logging all candidate evaluations, the projected surfaces in [[eq:proj_surface]] reflect the /true/ objective geometry explored by the optimizer, not a post-hoc approximation.


** Parameter-Cloud Geometry

<<sec:param_geometry_math>>
While the projected surfaces above characterize the /score landscape/, the geometric organization of the optimizer's /solutions/ provides complementary insight. Given $N$ patches with globally optimized parameters $\{\theta_j^g\}$ and locally re-optimized parameters $\{\theta_j^\ell\}$, we define the /parameter shift/ $\delta_j = \theta_j^\ell - \theta_j^g \in \R^7$.

We analyze these clouds via PCA [cite:jolliffe2016pca]. Let $\bar\theta^g$ and $\bar\theta^\ell$ denote the cloud centroids; the /centroid shift vector/ is $\Delta\bar\theta = \bar\theta^\ell - \bar\theta^g$. Its magnitude $\norm{\Delta\bar\theta}$ (in standardized units) quantifies how far local optimization displaces the typical solution. The covariance matrices $\Sigma^g, \Sigma^\ell$ define 2-sigma ellipsoids whose overlap measures the structural similarity of the two solution sets. For nonlinear structure we additionally apply t-SNE [cite:vandermaaten2008tsne] and UMAP [cite:mcinnes2018umap].
...
The core of our segmentation engine is an external force field derived from image gradients. Direct use of $\grad I$ is susceptible to high-frequency noise (dust, sensor grain, leaf hairs, compression artifacts). We instead form a smoothed /energy landscape/ that amplifies lesion structures while diffusing irrelevant texture.
...
where:
  ...
  - $E^{(t)}$ is the evolving energy field,
  - $\mu$ plays the role of viscosity (smoothing),
  - $(\lambda+\mu)\,\mathrm{div}(\grad E)$ is a coarse elastic coupling term,
  - $F$ is a forcing term derived from gradient discrepancies (see below),
  - $q$ is an edge-gating mask (strong edges should dominate forcing).



**** Seed vs.\ Image Gradients.

We define /image gradients/ $(I_x,I_y)$ as the high-frequency edge evidence, and a /seed/prior gradient field/ $(S_x,S_y)$ as a low-pass (Gaussian-smoothed) version of $(I_x,I_y)$. Then:
...
*** Chromatic Separability ($\mathcal{C}$)
...
We apply penalties to prevent degenerate solutions:
  ...
  - $\mathcal{P}_{\mathrm{area}}$: penalizes masks that are empty or cover an implausibly large fraction of the leaf.
  - $\mathcal{P}_{\mathrm{noise}}$: penalizes ultra-small components (dust/sensor noise).
...
\FOR{$i=1$ to $K$}
    ...
    \STATE $S_i \leftarrow S(I;\theta_i)$ using Eq. [[eq:objective]]
...
Prior to optimization, each leaf image undergoes automatic background removal via entropy-based masking and foreground isolation, producing a binary leaf ROI mask. Within the ROI, color calibration follows best practices for plant phenotyping imagery [cite:berry2018color]: white-balance normalization and consistent illumination geometry ensure that Lab color distances remain interpretable across acquisition sessions.
...
The evaluation uses a mixed framework of no-ground-truth and sparse-annotation metrics:

  - *No-GT (all images):* objective score distributions, convergence rate, runtime, lesion count/area histograms.
  - *Sparse GT (subset):* a stratified subset of images is used for Dice/IoU, Boundary-F1, and Hausdorff distance where teacher pseudo-labels serve as reference.
  - *Cross-condition generalization:* images are stratified by date/camera tray/genotype to test robustness, not IID random splits.
...
For completeness, classical segmentation approaches such as Lab-threshold, excess-green index, and Otsu-based methods serve as baselines for this unsupervised pipeline. These unsupervised baselines require no annotation and are evaluated on the same 174-image set. Comparison against externally annotated lesion benchmarks is reserved as future work.
...
In addition to Dice/IoU, we report:
  ...
  - Boundary F-score (BFScore) for edge fidelity (halo boundaries).
  - Connected-component metrics: lesion count error, area distribution (see Section [[sec:param_geometry_results]] and Table [[tab:patch_morph_transfer]]).
  - Morphological plausibility: area fraction, solidity, and eccentricity, which are phenotyping-relevant traits that can diverge from Dice even when overlap is high.
...
#+NAME: fig:optimizer_score_hist
[[file:figures/optimizer_score_hist.png]]


To guide the reader through the evidence, Table [[tab:claim_evidence]] maps the primary claims of this paper to the specific figures and tables that support them.
...
\midrule
The UNet student accurately mimics optimizer pseudo-labels (secondary). & Mean Dice $0.642\pm0.163$ (median 0.686) on teacher-nonempty images. & Section~\ref{sec:distillation_results} \\
\bottomrule
\end{tabular}
\end{table*}
#+END_SRC
...
<<sec:distillation_results>>
As a secondary application of the optimizer pseudo-labels, we trained a lightweight UNet student to mimic the optimizer's masks without any manual annotation. Across $N=174$ images, the baseline stitched UNet student (no ROI gate) achieves mean Dice $0.598\pm0.226$ (median 0.682). Conditioned on teacher-nonempty images ($N=162$), metrics become mean Dice $0.642\pm0.163$ (median 0.686) and mean IoU $0.491\pm0.155$ (median 0.522). The threshold sweep identifies 0.30 as the best operating point (Dice$_{\text{mean,all}}=0.604$). Full threshold sweep details and failure-tail analysis are provided in Appendix [[sec:appendix_distillation_details]].
...
To characterize the objective landscape that the stochastic optimizer navigates, we conducted a representative full-budget rerun on nine selected leaves with candidate-level trace logging (Section [[sec:trace_surfaces]]). We selected three high-score, three median-score, and three low-score leaves from the 174-image production run, ensuring coverage of the score distribution extremes and center. Importantly, this rerun used the /identical/ optimizer settings as the production run (budget $K{=}64$ random candidates, top-$4$ refinement with $R{=}12$ steps per seed, downscale $s{=}0.75$, same search bounds and objective weights), so results are directly comparable. The only additions were per-candidate trace logging and restriction to the nine-leaf manifest; the optimization logic was otherwise unchanged.


*** Score Surfaces

For each leaf, we visualize projected pairwise surfaces $\mathcal{S}_{p,q}$ (Eq. [[eq:proj_surface]]) for the three parameter pairs with the strongest influence on segmentation behavior: energy threshold $T$ vs.\ balloon force $\beta$, $T$ vs.\ viscosity $\mu$, and $\beta$ vs.\ inflation $\gamma$. Figure [[fig:surface_high]] shows the $T$—$\beta$ surface for a representative high-score leaf; Figure [[fig:surface_med]] shows the same for a median-score leaf; Figure [[fig:surface_low]] for a low-score leaf.
...
#+CAPTION: Trace-based projected objective surface ($T$ vs.\ $\beta$) for a representative /high-score/ leaf (DSC_0198). All 112 evaluated candidates are shown as scatter points colored by score; the final optimum (red star), best random-phase point (orange triangle), and best refine-phase point (cyan square) are marked. A fitted triangulated surface is overlaid where sampling density permits.
#+NAME: fig:surface_high
[[file:figures/surface_DSC_0198_segment_1_segmented_smoothed__surface_energy_threshold_vs_beta.png]]



#+CAPTION: Trace-based projected objective surface ($T$ vs.\ $\beta$) for a representative /median-score/ leaf (DSC_0379).
#+NAME: fig:surface_med
[[file:figures/surface_DSC_0379_segment_1_segmented_smoothed__surface_energy_threshold_vs_beta.png]]



#+CAPTION: Trace-based projected objective surface ($T$ vs.\ $\beta$) for a representative /low-score/ leaf (DSC_0261). The surface is flatter than for high-score leaves, consistent with a landscape that offers limited exploitable curvature.
#+NAME: fig:surface_low
...
Figure [[fig:trajectory_high]] shows the convergence of best-score-so-far for a high-score leaf, demonstrating the transition from random exploration (blue) to targeted refinement (orange). The refine phase achieves a distinct score improvement, confirming that local search around top random candidates is beneficial for morphologically complex leaves. For low-score leaves, the best-so-far curve plateaus early during random sampling, and the refine phase produces negligible gains.


#+CAPTION: Convergence trajectory: best score so far vs.\ candidate index for a high-score leaf (DSC_0198). Random-phase candidates (blue region) establish initial optima; refine-phase candidates (orange region) improve upon them.
...
<<sec:param_geometry_results>>

Complementing the score-landscape analysis above, we characterize the /geometric organization/ of the optimizer's solutions in the 7-dimensional parameter space (Section [[sec:param_geometry_math]]). Using 150 global—local parameter pairs from 15 leaves (75 interior, 75 boundary patches), we project parameter vectors via PCA and nonlinear embeddings.


*** PCA Separation of Global and Local Optima

Figure [[fig:pca2d_global_local]] shows the PCA 2D projection of all global (full-leaf) and local (patch-level) parameter vectors, colored by Dice improvement $\Delta D$. Two principal components capture 46.5% of the total variance. Notably, PC1 separates the global and local clouds with a centroid shift magnitude of 1.616 standardized units, confirming that local re-optimization produces a /systematic, directional/ displacement in parameter space rather than random perturbation.
...
To visualize the volumetric organization of solution clouds, we fit 2-sigma covariance ellipsoids and convex hulls in PCA space (Figure [[fig:cov_ellipsoids]]). The global and local clouds form overlapping but displaced ellipsoids whose orientations are similar, suggesting that the optimization landscape has comparable curvature at both scales. The centroid shift arrow indicates the direction of systematic displacement. Patches where local optimization improved Dice ($\Delta D > +0.02$; $n{=}73$) and patches where it hurt ($\Delta D < -0.02$; $n{=}48$) show partial separation in PCA space, suggesting that the /direction/ of the parameter shift, not just its magnitude, encodes information about optimization success.
...
Figure [[fig:qual_grid]] organizes qualitative results across the Dice spectrum, spanning five biologically meaningful lesion categories:

  - Small discrete necrotic spots (strong edges),
  - Chlorotic halos (soft edges),
  - Coalescing/merged lesions,
  - Mixed inner/outer structure (necrotic core + halo ring),
  - Leaf venation confounds and glare artifacts.
...
<<sec:multistage>>
Broad lesions frequently contain inner oscillating or necrotic substructures that are biologically distinct from the outer chlorotic halo. We address this with a *two-stage nested inference strategy*:

  1. Stage-A (outer): run optimizer to obtain outer lesion regions $M_{\text{outer}}$.
  2. Stage-B (inner): restrict to $M_{\text{outer}}$ and re-run with a higher threshold $T'$ and slightly different snake rigidity to capture necrotic cores $M_{\text{inner}}$.

This yields biologically meaningful /nested masks/ enabling traits such as:
...
\STATE *Stage-A (Outer):* $\theta_A^\star \leftarrow \mathrm{Optimize}(I;\Theta)$
\STATE $M_{\text{outer}} \leftarrow \mathrm{RunPhysics}(I;\theta_A^\star)$ \COMMENT{green overlay}
\IF{$|M_{\text{outer}}| > 0$}
    \STATE *Stage-B (Inner):* Restrict search to pixels inside $M_{\text{outer}}$
...
Table [[tab:patch_morph_transfer]] summarizes the results.
...
Overlap metrics (Dice, IoU) show only modest changes ($+0.013$ mean $\Delta$Dice), confirming that morphological fidelity and pixel-overlap can diverge and should be reported jointly in phenotyping applications [cite:ronneberger2015u].
This distinction is especially important in high-throughput phenotyping workflows where downstream trait extraction—-lesion counts, area distributions, and necrotic ratios—-constitutes the primary scientific output [cite:gehan2017high,plantcv2025]; a segmentation with high Dice but biased morphological statistics can mislead quantitative trait analysis.
...
To complement the main results, we report additional summary statistics of the stochastic optimizer, including output morphology distributions (mask area and component counts) and the empirical distributions of optimized parameters. Such diagnostics are useful in high-throughput phenotyping pipelines where downstream traits (e.g., lesion counts and area distributions) are primary outputs, and not fully characterized by overlap metrics alone (cf.\ PlantCV-style phenotyping workflows and trait extraction [cite:plantcv2025]).
...
<<sec:appendix_geometry>>
This section presents additional parameter-cloud geometry plots from the manifold and solid-3D analyses described in Section [[sec:param_geometry_results]].
...
<<sec:appendix_distillation_details>>
This appendix provides the full threshold sweep results and failure-tail analysis for the teacher—student distillation pipeline, moved here from the main results section to maintain focus on the primary optimizer and geometry contributions.


** Threshold Sweep

The threshold sweep was conducted over the range $[0.10, 0.60]$ on stitched probability maps. The sweep best row occurs at threshold=0.30:

  - $\text{Dice}_{\text{mean,all}}=0.604$, $\text{IoU}_{\text{mean,all}}=0.463$
  - ROI-gated at threshold 0.30: mean Dice $0.599\pm0.225$ (all), $0.644\pm0.160$ (teacher-nonempty)

Dice decreases smoothly with increasing threshold; teacher-empty disagreements persist across all thresholds in the sweep, indicating that threshold tuning alone does not resolve this failure mode.
...
Failure-tail counts remain similar across the baseline and ROI-gated variants:

  - Baseline: 18 cases with Dice$<0.3$, including 12 teacher-empty/student-nonempty disagreements.
  - ROI-gated: 19 cases with Dice$<0.3$, including 12 teacher-empty/student-nonempty disagreements.

The small baseline-to-ROI metric shift also suggests disagreements are not primarily driven by background leakage outside the leaf ROI. These cases may reflect either optimizer false negatives under the inner-mask definition or a mismatch in necrosis/halo semantics between teacher and student.
