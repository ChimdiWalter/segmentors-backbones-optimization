...
# Search bounds (tighter PDE/snake ranges)
BOUNDS = {
    "mu":              (0.05, 0.60),
    "lambda":          (0.05, 0.60),
...
# Objective weights / constraints (for ~5MP leaves)
TARGET_AREA_FRAC    = 0.08      # prefer ~8% lesion pixels on average
AREA_FRAC_TOL       = 0.05      # ±5% band → [3%, 13%] OK
W_AREA              = 0.05      # soft area penalty
W_SMALL_BASE        = 0.2       # base penalty per ultra-tiny speckle
SMALL_MIN_AREA      = 16        # minimal "non-tiny" lesion area
...
W_CONTOUR           = 1.0       # reward multiple non-tiny closed contours (log scale)
...
def evaluate_params_refined(
    ...
    small_thresh = max(SMALL_MIN_AREA, int(2e-6 * img_area))
    ...
    # area penalty with tolerance band
