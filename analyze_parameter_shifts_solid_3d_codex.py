...
Inputs:
  experiments/exp_v1/outputs/patch_localven_opt_v2.csv (if present)
  experiments/exp_v1/outputs/patch_local_opt_v2.csv (fallback)
  experiments/exp_v1/outputs/patch_transfer_morphology_eval.csv

Output:
  experiments/exp_v1/outputs/parameter_shift_3d_analysis_solid_codex/
"""
...
V2_CSV_CANDIDATES = [
    os.path.join(REPO, "experiments/exp_v1/outputs/patch_localven_opt_v2.csv"),
    os.path.join(REPO, "experiments/exp_v1/outputs/patch_local_opt_v2.csv"),
]
MORPH_CSV = os.path.join(REPO, "experiments/exp_v1/outputs/patch_transfer_morphology_eval.csv")
MONTAGE_ANALYSIS_CSV = os.path.join(
    REPO, "experiments/exp_v1/outputs/patch_transfer_morphology_montages/patch_transfer_morphology_analysis.csv"
)
OUT_DIR  = os.path.join(REPO, "experiments/exp_v1/outputs/parameter_shift_3d_analysis_solid_codex")
...
# ── Part 1: Merged CSV ──
def load_and_merge():
    v2_path = next((p for p in V2_CSV_CANDIDATES if os.path.exists(p)), None)
    if v2_path is None:
        raise FileNotFoundError(
            "None of expected patch CSVs found: "
            + ", ".join(os.path.relpath(p, REPO) for p in V2_CSV_CANDIDATES)
        )
    ...
    if os.path.exists(MONTAGE_ANALYSIS_CSV):
        extra = pd.read_csv(MONTAGE_ANALYSIS_CSV)
        keep = [c for c in ["patch_image", "patch_type", "content_frac"] if c in extra.columns]
        if "patch_image" in keep:
            df = df.merge(extra[keep].drop_duplicates("patch_image"), on="patch_image", how="left", suffixes=("", "_extra"))
            if "patch_type_extra" in df.columns:
                df["patch_type"] = df["patch_type"].fillna(df["patch_type_extra"])
                df.drop(columns=["patch_type_extra"], inplace=True)
            if "content_frac_extra" in df.columns:
                df["content_frac"] = df["content_frac"].fillna(df["content_frac_extra"])
                df.drop(columns=["content_frac_extra"], inplace=True)
    out = os.path.join(OUT_DIR, "parameter_shift_analysis_solid_codex.csv")
    df.to_csv(out, index=False)
    print(f"  Input patch CSV: {v2_path}")
...
def save_plotly(fig_pl, name):
    """Save plotly figure as html (static PNG is handled by matplotlib)."""
...
def plot_pca_hulls(G_pca, L_pca, df):
    ...
    save_fig(fig, "pca3d_convex_hull_global_local_codex")
    ...
    try:
        ...
        save_plotly(fig_pl, "pca3d_convex_hull_global_local_codex")
...
def plot_pca_ellipsoids(G_pca, L_pca, df):
    ...
    ax.legend(fontsize=9)
    save_fig(fig, "pca3d_covariance_ellipsoids_codex")
...
def plot_pca_density(G_pca, L_pca, df):
    ...
    try:
        ...
        fig_pl.update_layout(scene=dict(xaxis_title="PC1", yaxis_title="PC2", zaxis_title="PC3"),
                             ...
                             width=1000, height=800)
        save_plotly(fig_pl, "pca3d_density_isosurfaces_codex")
        ...
        ax.legend(fontsize=9)
        save_fig(fig, "pca3d_density_isosurfaces_codex")
    except Exception as e:
...
def plot_delta_hulls_patchtype(df):
    ...
    ax.legend(fontsize=9)
    save_fig(fig, "delta3d_hulls_by_patchtype_codex")
    ...
    try:
        ...
        save_plotly(fig_pl, "delta3d_hulls_by_patchtype_codex")
    except Exception:
        pass
...
def plot_dD_surfaces(G_pca, L_pca, df):
    ...
    ax.legend(fontsize=9)
    save_fig(fig, "pca3d_positive_vs_negative_dD_surfaces_codex")
    ...
    try:
        ...
        save_plotly(fig_pl, "pca3d_positive_vs_negative_dD_surfaces_codex")
    except Exception:
        pass
...
def plot_raw_hulls_tbm(df):
    ...
    save_fig(fig, "raw3d_threshold_beta_mu_hulls_codex")
    ...
    try:
        ...
        save_plotly(fig_pl, "raw3d_threshold_beta_mu_hulls_codex")
    except Exception:
        pass
...
def plot_raw_hulls_tgd(df):
    ...
    ax.legend(fontsize=9, loc="upper left")
    save_fig(fig, "raw3d_threshold_gamma_diffusion_hulls_codex")
    ...
    try:
        ...
        save_plotly(fig_pl, "raw3d_threshold_gamma_diffusion_hulls_codex")
    except Exception:
        pass
...
def write_summary(df, G_pca, L_pca):
    ...
    lines.append("")
    lines.append("A) pca3d_convex_hull_global_local_codex: Convex hulls around all global")
    lines.append("   (blue) and local (red) parameter vectors in PCA space.")
    lines.append("   Gray lines connect each global->local pair.")
    lines.append("")
    lines.append("B) pca3d_covariance_ellipsoids_codex: 2-sigma covariance ellipsoids")
    ...
    lines.append("")
    lines.append("C) pca3d_density_isosurfaces_codex: KDE density isosurfaces in PCA space.")
    lines.append("   Shows where parameter vectors concentrate most densely.")
    lines.append("")
    lines.append("D) delta3d_hulls_by_patchtype_codex: Convex hulls and 1.5-sigma ellipsoids")
    lines.append("   of delta parameters (d_energy_threshold, d_beta, d_mu)")
    lines.append("   split by interior (green) vs boundary (orange).")
    lines.append("")
    lines.append("E) pca3d_positive_vs_negative_dD_surfaces_codex: Hulls/ellipsoids")
    lines.append("   separating local-theta vectors where dD > +0.02 (red)")
    lines.append("   from dD < -0.02 (blue).")
    lines.append("")
    lines.append("F) raw3d_threshold_beta_mu_hulls_codex: Convex hulls in raw parameter")
    lines.append("   space (energy_threshold x beta x mu). Arrows colored by dD.")
    lines.append("")
    lines.append("G) raw3d_threshold_gamma_diffusion_hulls_codex: Same style for")
    ...
    lines.append("     confirming substantial overlap (see delta3d_hulls_by_patchtype_codex).")
    lines.append("")
    ...
    lines.append("  -> See pca3d_positive_vs_negative_dD_surfaces_codex for cluster separation.")
    lines.append("")
    ...
    txt = "\n".join(lines)
    out = os.path.join(OUT_DIR, "solid_3d_summary_codex.txt")
