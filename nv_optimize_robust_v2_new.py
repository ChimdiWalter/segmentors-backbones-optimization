#!/usr/bin/env python3
# nv_optimize_robust_v2.py (refactored + scale-aware + grad/color-aware)
...
BOUNDS = {
    ...
    "beta":            (0.50, 2.0),
    "gamma":           (0.10, 0.50),
    "energy_threshold": (5, 70),  # integer
}

# Objective weights / constraints (tuned for large ~5MP images)
TARGET_AREA_FRAC    = 0.08      # prefer ~2% lesion pixels (you can adjust), formerly 0.5
W_AREA              = 0.05       # penalty weight for area deviation, formerly 0.1
W_SMALL_BASE        = 0.2      # base penalty per ultra-tiny speckle
SMALL_MIN_AREA      = 25        # never treat <20 px as non-small
MAX_AREA_FRAC_HARD  = 0.70      # hard cap (reject if above)
MIN_AREA_FRAC_HARD  = 1e-6      # hard cap (reject if below)

# New weights for gradient + color + contour terms
W_GRAD_ALIGN        = 1.0       # reward edges of mask that align with image gradients
W_COLOR_DIST        = 3.0       # reward brown-vs-green separation in Lab space
W_CONTOUR           = 1.0       # reward multiple non-tiny closed contours
W_OUTLIER           = 0.2       # discourage wildly off-scale blobs
...
def snake_seg(
    ...
    beta: float,
    gamma: float,
    min_blob_frac: float = 2e-6, #1e-5
    max_blob_frac: float = 0.30, #0.1
    energy_threshold: int = 50,
) -> np.ndarray:
    """
    Scale-aware snake: lesion size bounds are specified as fractions of full image area.
    """
    h, w = image_u8.shape
    img_area = h * w

    # convert fractional bounds to absolute area
...
def fused_channel_u8(bgr8: np.ndarray, binary_mask: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    # MODIFIED: Added lab[..., 2] (The 'B' channel: Blue vs Yellow)
    # This captures yellowing/browning much better than just Green/Gray
    ...
    for ch in candidates:
        ...
        # We take the MAXIMUM response across channels to ensure we don't miss anything
...
# ----------------------------- Gradient/Color Metrics -------------------------
def grad_alignment_score(seg_mask_bool: np.ndarray,
                         grad_mag_u8: np.ndarray) -> float:
    """
    How well do mask edges sit on strong image gradients?
    Returns a normalized score ~[0, +inf), where 1.0 means
    'edges are on gradients about as strong as average'; >1.0 means better.
    """
...
def color_distance_score(seg_mask_bool: np.ndarray,
                         img_lab: np.ndarray,
                         img_u8: np.ndarray,
                         fg_min_pixels: int = 50) -> float:
    """
    Measure how different the lesion color (inside mask) is from the
    surrounding leaf (outside mask but inside leaf) in Lab (a*, b*) space.
    Higher = lesions are more chromatically distinct (brown vs green).
...
def evaluate_params_refined(
    ...
    its_snake: int,
    params: Dict,
    min_blob_frac: float = 1e-5,
    max_blob_frac: float = 0.10,
) -> EvalResult:
    """
    Run refined energy + snake; return composite score.
    Uses precomputed image + seed gradients and scale-aware lesion size,
    plus gradient alignment and color distance metrics.
    """
    h, w = img_u8.shape
    img_area = h * w
    ...
    # Snake segmentation with fractional bounds
    ...
    # ultra tiny speckles (relative to full image)
    small_thresh = max(SMALL_MIN_AREA, int(1e-5 * img_area))
    small_count  = int(np.sum(areas < small_thresh))

    # scale-normalized blob sizes
    median_area = float(np.median(areas))
    size_norm   = areas / (median_area + 1e-8)

    # blobs that are way smaller or larger than the dominant size
    outlier_mask  = (size_norm < 0.25) | (size_norm > 4.0)
    outlier_count = int(np.sum(outlier_mask))

    valid_count = int(np.sum(areas >= small_thresh))

    # gradient alignment & color distance
    grad_align = grad_alignment_score(seg_mask_bool, grad_mag_u8)
    color_dist = color_distance_score(seg_mask_bool, lab_img, img_u8)

    W_SMALL = W_SMALL_BASE

    score = (
        W_GRAD_ALIGN * grad_align +
        W_COLOR_DIST * color_dist +
        W_CONTOUR * valid_count -
        W_SMALL * small_count -
...
def process_one_image(
    ...
    # Downscale for search; keep color aligned
...
def _finalize(
    ...
    eps = 1e-8
    grad_mag_f_u8 = np.uint8(255.0 * img_mag_f / max(float(img_mag_f.max()), eps))
    bgr8_for_lab = _to_uint8(bgr8_full)
    lab_full = cv2.cvtColor(bgr8_for_lab, cv2.COLOR_BGR2Lab)
    ...
    res_final = evaluate_params_refined(
        ...
        grad_mag_u8=grad_mag_f_u8,
        lab_img=lab_full,
        its_snake=snake_iters_final,
        params=params,
    )
...
def main():
    ...
    print("========== nv_optimize_robust_v2 (refined, scale-aware, grad/color-aware) ==========")
