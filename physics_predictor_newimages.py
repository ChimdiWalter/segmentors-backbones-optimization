#!/usr/bin/env python3
# run_fast_physics.py
# Uses the trained Neural Net to instantly pick parameters, 
# then runs the Physics ONCE to generate the mask.
...
MODEL_PATH = "./param_predictor_resnet.pth" 
# Folder containing images to process
INPUT_DIR = "/deltos/c/maize/image_processing/data/leaf/22r/aleph/18.7"
# Where to save the results
OUTPUT_DIR = "fast_physics_results"

# Visualization Color (Light Pink)
COLOR_INNER = (255, 100, 255) 

# ================= 1. MODEL SETUP =================
def get_model():
    # Must match the training script exactly (ResNet18 -> 7 outputs)
...
def predict_params(model, device, img_bgr):
    # Prepare image for ResNet (Resize 224x224, Normalize)
    ...
    # Format parameters (Undo the scaling x0.01 we did during training)
    params = {
        "mu": float(preds[0]),
        ...
        "beta": float(preds[4]),
        "gamma": float(preds[5]),
        "energy_threshold": int(preds[6] * 100.0) # Scale back up
    }
    return params

# ================= 2. PHYSICS ENGINE (Condensed) =================
...
def run_pde(seed_mag, img_mag, igx, igy, sgx, sgy, p):
    ...
    # Run 20 iterations (fast but effective)
    for _ in range(20):
...
def run_snake(img_u8, energy, p):
    ...
    for rgn in regionprops(labeled):
        if rgn.area < 16: continue # Ignore tiny noise
...
def fused_channel(bgr):
    ...
    candidates = [bgr[...,1], gray, lab[...,1], lab[...,2]] # G, Gray, a, b
    fused = None
    for c in candidates:
        if c.dtype!=np.uint8: c = _to_uint8(c)
        # CLAHE boost for details
...
def main():
    ...
    print(f"Running Fast Physics on {len(files)} images...")
    
    for i, fname in enumerate(files):
        path = os.path.join(INPUT_DIR, fname)
        
        # 1. Load Raw
        raw = cv2.imread(path, -1) # Any depth (for overlay)
        bgr = cv2.imread(path, 1)  # 8-bit BGR (for Neural Net input)
        
        if bgr is None: 
            print(f"Skipping {fname} (Load error)")
            continue
        
        # 2. Predict Params (The Magic Step)
        params = predict_params(model, device, bgr)
        # Debug print to ensure model is varying parameters
        if i % 5 == 0:
            print(f"[{i+1}/{len(files)}] {fname} -> Thresh:{params['energy_threshold']} Beta:{params['beta']:.3f}")
        
        # 3. Prepare Physics Input (Fused Channel)
        ...
        # Mask (16-bit White/Black)
        mask16 = (mask_bool.astype(np.uint16)) * 65535
        
        base = os.path.splitext(fname)[0]
        cv2.imwrite(os.path.join(OUTPUT_DIR, f"{base}_pred_overlay.tif"), overlay)
        cv2.imwrite(os.path.join(OUTPUT_DIR, f"{base}_pred_mask.tif"), mask16)
