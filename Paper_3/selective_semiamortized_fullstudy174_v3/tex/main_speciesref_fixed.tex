...
On 174 diseased maize leaves with 7 physics parameters,
...
that are comparable to the optimizer objective on this test split.
...
with learned update rules. Related test-time adaptation methods~\cite{wang2020tent}
also perform post-hoc instance-level updates, although their objectives differ
from semi-amortized latent-variable refinement.
...
phenotyping platforms~\cite{ubbens2017deep,li2014review}.
...
174 diseased maize leaf images ($1920 \times 1080$, 16-bit TIFF,
...
\paragraph{Score recovery relative to the optimizer objective.}
The medium-budget refinement achieves a higher mean score (18.24) than the
stored full-optimizer objective value (18.05) on this 35-leaf test split.
This indicates that the amortized initialization can reach alternative local
optima under the same scoring function. We interpret this as evidence of useful
warm starts, not as a general guarantee of superiority over exhaustive
optimization.
...
refinement (12 steps) reaches a higher mean objective value than the stored
full-optimizer result on this test split (18.24 vs.\ 18.05), indicating that
the amortized starting point can reach alternative local optima.}
\label{fig:score_recovery}
\end{figure}

\subsection{Qualitative Leaf Cases}

To complement aggregate metrics, we include real-leaf qualitative panels from
both the primary 174-image maize set and the external 18.7 set.
Figure~\ref{fig:qual_primary} shows representative direct/refine/fallback
routing cases with predicted, refined, and oracle parameter summaries.
Figure~\ref{fig:qual_failure} highlights success and difficult cases used in
the failure taxonomy. Figure~\ref{fig:qual_external} shows external 18.7 leaves
with uncertainty and inferred routing; oracle parameter vectors are unavailable
for this external batch.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{fig16_representative_leaf_routing.pdf}
\caption{Representative qualitative panel on real maize leaves from the primary
set. Each tile reports uncertainty, routing decision, and
\(\theta\)-vector summaries (predicted/refined/oracle).}
\label{fig:qual_primary}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{fig17_failure_case_montage.pdf}
\caption{Failure-case montage on real primary-set maize leaves: low-uncertainty
success, medium-uncertainty refine case, high-uncertainty fallback case, and a
hard phenotype-extreme case.}
\label{fig:qual_failure}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{fig18_external_18p7_panel.pdf}
\caption{External 18.7 qualitative panel with real maize leaf images. Uncertainty
and inferred routing are shown under the best policy thresholds
\((q_1{=}0.5, q_2{=}0.7)\). Oracle parameter vectors are unavailable.}
\label{fig:qual_external}
\end{figure}
...
\subsection{Score Recovery Relative to Stored Optimizer Results}

The finding that medium-budget refinement reaches a higher mean objective
value than the stored full-optimizer result on this split (18.24 vs.\ 18.05)
suggests that the amortized predictor provides useful inductive bias for the
refinement search. The learned initialization can explore regions of parameter
space that the random-restart trajectory did not retain in the logged optimum.

This has implications for meta-learning and warm-starting optimization:
even imperfect learned predictions can improve optimization from random starts
by providing better initialization in some regimes.
...
On 174 diseased maize leaves with 7 physics parameters, the selective
...
performance. On this test split, medium-budget refinement reaches a higher
mean objective value than the stored optimizer result, consistent with the
value of learned initialization for warm-started search.
...
\bibliography{../bib/references_speciesref_fixed}
