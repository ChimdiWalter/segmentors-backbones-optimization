...
OUTPUT_DIR = '/deltos/e/lesion_phes/code/python/pipeline/segmentors_backbones/navier_output_fused_auto_parallel'
...
# ---- curated list of 6 "good" combos (mu, lam, dr, alpha, beta, gamma) ----
COMBOS = [
...
def snake_seg(image_u8, energy_u8, its, alpha, beta, gamma, l_size, u_size, energy_threshold):
    ...
    for rgn in props:
        ...
        s = np.linspace(0, 2*np.pi, 200, endpoint=False)  # 200-pt init
...
def process_single_image(filename):
    ...
    try:
        ...
        # Fused channel + CLAHE on-leaf
        fused_u8 = fused_channel_u8(bgr8, binary_mask)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        fused_enh = clahe.apply(fused_u8)
        masked_fused = cv2.bitwise_and(fused_enh, fused_enh, mask=binary_mask)
...
def main():
    # keep each worker single-threaded (avoid oversubscription)
