#!/usr/bin/env python3
"""Patch-level local optimization experiment (v2).

Fixes over v1:
  1. Padded pixels on boundary patches are excluded from Dice/IoU scoring
     via a valid-pixel mask built from pad_right/pad_bottom metadata AND
     the grayscale nonzero region.
  2. Mostly-background patches (< 10% nonzero pixels) are skipped entirely.
...
Usage:
  ...
  python3 patch_local_optimize_compare_v2.py --n_leaves 15 --patches_per_leaf 10 \
      --seed 0 --budget 32 --refine 6 \
      --out_csv experiments/exp_v1/outputs/patch_local_opt_v2.csv
"""
...
# Minimum nonzero-pixel fraction to keep a patch (skip mostly-background)
MIN_CONTENT_FRAC = 0.10
...
# ── Valid-pixel mask and masked metrics ──────────────────────────────────────

def build_valid_mask(gray8, pad_right, pad_bottom):
    """Build boolean mask of valid (non-padded, non-background) pixels.

    Combines two constraints:
      - Geometry: exclude the rightmost `pad_right` cols and bottommost
        `pad_bottom` rows (reflection-padded region from patchification).
      - Content: exclude pixels where grayscale == 0 (black background
        outside the leaf ROI).
    """
    h, w = gray8.shape
    ...
    # Also exclude true-black background pixels
    valid &= (gray8 > 0)
    return valid


def dice_masked(a, b, valid, eps=1e-6):
    """Dice coefficient computed only within valid-pixel region."""
...
def iou_masked(a, b, valid, eps=1e-6):
    """IoU computed only within valid-pixel region."""
    a = a.astype(bool) & valid
    b = b.astype(bool) & valid
    inter = float((a & b).sum())
    union = float((a | b).sum())
...
def make_leaf_montage(patch_results, leaf_id, out_path):
    """Montage for one leaf: 3 rows (best improvement, median, worst).

    Each row: raw | teacher overlay | global overlay | local overlay
    """
    if not patch_results:
        return

    sorted_by_delta = sorted(patch_results,
                             key=lambda r: r["dice_local_teacher"] - r["dice_global_teacher"])
    ...
    for pr in picks:
        ...
        def resize_mask(m):
            return cv2.resize(m.astype(np.uint8) * 255, (TW, TH),
                              interpolation=cv2.INTER_NEAREST) > 127

        mt = resize_mask(pr["m_teacher"])
        mg = resize_mask(pr["m_global"])
        ml = resize_mask(pr["m_local"])
        ...
        lbl = f"dD={delta:+.3f}"
        cv2.putText(ov_local, lbl, (2, TH - 4), cv2.FONT_HERSHEY_SIMPLEX,
                    0.35, (255, 255, 255), 1)
    ...
    label_h = 20
    header = np.ones((label_h, TW * 4, 3), dtype=np.uint8) * 255
    for i, txt in enumerate(["Raw", "Teacher", "Global", "Local"]):
        cv2.putText(header, txt, (i * TW + 4, 14),
...
def main():
    ap = argparse.ArgumentParser(
        description="Patch-level local optimization comparison (v2: valid-pixel scoring)")
    ap.add_argument("--n_leaves", type=int, default=15)
    ...
    ap.add_argument("--out_csv", type=str,
                    default=str(ROOT / "experiments" / "exp_v1" / "outputs" / "patch_local_opt_v2.csv"))
    ...
    common_leaves = sorted(k for k in by_leaf if k in leaf_params)
    ...
    print(f"Selected {len(selected_leaves)} leaves")

    # ── Prepare output ───────────────────────────────────────────────────
    os.makedirs(args.out_dir, exist_ok=True)
    fieldnames = [
        "leaf_id", "patch_image", "patch_mask", "x", "y", "pad_right", "pad_bottom",
        "patch_type", "content_frac",
    ...
    total_target = args.patches_per_leaf * len(selected_leaves)
    done = 0

    for leaf_idx, leaf_stem in enumerate(selected_leaves):
        ...
        # Shuffle before filtering to avoid bias
        rng.shuffle(interior)
        rng.shuffle(boundary)

        n_want_int = args.patches_per_leaf // 2  # 5
        n_want_bnd = args.patches_per_leaf - n_want_int  # 5

        leaf_results_for_montage = []

        def process_pool(candidates, patch_type, n_want):
            nonlocal done, skipped_bg, skipped_io
            collected = 0
            for patch_row in candidates:
                if collected >= n_want:
                    break
                ...
                # ── FIX 2: Skip mostly-background patches ────────────
                content_frac = float((gray8 > 0).sum()) / (gray8.shape[0] * gray8.shape[1])
                ...
                # M_global_on_patch
                m_global_inner, _ = finalize_inner_mask(work, bgr, params_global,
                                                        max_blob_frac=1.0)
                ...
                if m_local_inner is None:
                    ...
                    local_params = sample_candidate(rng)
                    local_score = -1e9

                # Compute metrics with valid-pixel masking
                d_global_teacher = dice_masked(m_global_inner, m_teacher, valid)
                d_local_teacher  = dice_masked(m_local_inner, m_teacher, valid)
                d_local_global   = dice_masked(m_local_inner, m_global_inner, valid)
                i_global_teacher = iou_masked(m_global_inner, m_teacher, valid)
                i_local_teacher  = iou_masked(m_local_inner, m_teacher, valid)
                ...
                row_out = {
                    ...
                    "content_frac": f"{content_frac:.4f}",
                ...
                collected += 1
                print(f"  [{done}/{total_target}] {os.path.basename(patch_img_path)} "
                      f"type={patch_type} content={content_frac:.2f} "
                      f"Dice(g,t)={d_global_teacher:.3f} "
                      f"Dice(l,t)={d_local_teacher:.3f} "
                      f"delta={d_local_teacher - d_global_teacher:+.3f} "
                      f"({elapsed:.1f}s)")

        print(f"\n[Leaf {leaf_idx+1}/{len(selected_leaves)}] {leaf_stem} "
              f"(int={len(interior)}, bnd={len(boundary)})")
        process_pool(interior, "interior", n_want_int)
        process_pool(boundary, "boundary", n_want_bnd)

        # Generate montage
        montage_path = Path(args.out_dir) / f"montage_patch_opt_v2_{leaf_stem}.png"
    ...
    print(f"Skipped: {skipped_bg} mostly-background, {skipped_io} unreadable")
    ...
    for r in all_rows:
        dg = float(r["dice_global_teacher"])
        dl = float(r["dice_local_teacher"])
        d = dl - dg
        all_delta.append(d)
        all_dice_global.append(dg)
        all_dice_local.append(dl)
        if r["patch_type"] == "interior":
            interior_delta.append(d)
            int_dice_global.append(dg)
            int_dice_local.append(dl)
        else:
            boundary_delta.append(d)
            bnd_dice_global.append(dg)
            bnd_dice_local.append(dl)
    ...
    print("  [v2: valid-pixel masking, background patches excluded]")
    print("=" * 72)

    print(f"\n  ALL patches (n={all_stats['n']}):")
    print(f"    mean delta  = {all_stats['mean']:+.4f},  median delta = {all_stats['median']:+.4f}")
    ...
    print(f"\n  INTERIOR patches (n={int_stats['n']}):")
    print(f"    mean delta  = {int_stats['mean']:+.4f},  median delta = {int_stats['median']:+.4f}")
    if int_dice_global:
        print(f"    Dice(global,teacher): mean={stats(int_dice_global)['mean']:.4f}")
        print(f"    Dice(local,teacher):  mean={stats(int_dice_local)['mean']:.4f}")

    print(f"\n  BOUNDARY patches (n={bnd_stats['n']}):")
    print(f"    mean delta  = {bnd_stats['mean']:+.4f},  median delta = {bnd_stats['median']:+.4f}")
    if bnd_dice_global:
        print(f"    Dice(global,teacher): mean={stats(bnd_dice_global)['mean']:.4f}")
        print(f"    Dice(local,teacher):  mean={stats(bnd_dice_local)['mean']:.4f}")
    ...
    for r in all_rows:
        ...
        leaf_deltas.setdefault(lid, []).append(d)
    for lid in sorted(leaf_deltas.keys()):
        ds = leaf_deltas[lid]
        s = stats(ds)
        print(f"    {lid}: n={s['n']}, mean delta={s['mean']:+.4f}, median={s['median']:+.4f}")
    ...
    if abs(mean_all) < 0.03:
        ...
        print(f"    Local re-optimization provides negligible benefit on average.")
    else:
        print(f"  * Mean improvement {mean_all:+.4f} >= 0.03:")
